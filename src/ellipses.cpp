...
inline float calculateDistance( const float x1, const float x2, const float y1, const float y2){
    return static_cast<float>(sqrt(pow(x1- x2, 2.0) + pow(y1 - y2, 2.0)));
}

inline float calculateDistanceSquared( const float x1, const float x2, const float y1, const float y2){
...
// Implements Randomized Hough Transform for Ellipse Detection
...
// Computes Hough transform twice, first for the larger ellipse, second time for the smaller one
std::pair<bool, Point> find2Ellipses(Mat & image, Mat & color_image){
    ...
    if(bestHorizontalEllipse.score/bestHorizontalEllipse.majorAxis > 1.2f && bestVerticalEllipse.score/bestVerticalEllipse.majorAxis > 1.2f && ellipsesDist < 5.0f && bestHorizontalEllipse.majorAxis > bestVerticalEllipse.majorAxis*1.3f) {
        result.first = true;
        result.second = Point(bestHorizontalEllipse.x, bestHorizontalEllipse.y);
        cv::ellipse(color_image, cv::Point(bestVerticalEllipse.x,bestVerticalEllipse.y),cv::Size(bestVerticalEllipse.minorAxis,bestVerticalEllipse.majorAxis),bestVerticalEllipse.angle,0,360,cv::Scalar(0,0,255));
        cv::ellipse(color_image, cv::Point(bestHorizontalEllipse.x,bestHorizontalEllipse.y),cv::Size(bestHorizontalEllipse.minorAxis,bestHorizontalEllipse.majorAxis),bestHorizontalEllipse.angle,0,360,cv::Scalar(255,0,0));
